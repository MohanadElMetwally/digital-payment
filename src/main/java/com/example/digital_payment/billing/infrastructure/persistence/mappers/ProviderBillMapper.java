...
public class ProviderBillMapper {
    public ProviderBills toDomain(ProviderBillResponse response) {
        ProviderBillSnapshot snapshot = new ProviderBillSnapshot(response.id(),
            response.customerNumber(), response.customerName(), response.amount(),
            response.currency(), response.provider(), response.status(),
            response.billingPeriodStart(), response.billingPeriodEnd(), response.dueDate(),
            response.createAt());
