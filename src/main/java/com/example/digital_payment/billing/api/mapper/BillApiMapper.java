...
public class BillApiMapper {
    public BillResponse toResponse(BillResult result) {
        return new BillResponse(result.id(), result.billerId(), result.userId(),
                result.externalCustomerNumber(), result.externalCustomerName(),
                result.externalBillId(), result.amount(), result.currency(),
                result.status().toString(), result.billingPeriodStart().toString(),
                result.billingPeriodEnd().toString(), result.dueDate().toString(),
                result.lastSyncedAt().toString(), result.createdAt().toString());
    }
}
