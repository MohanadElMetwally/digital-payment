...
public class CreditCardFacade {
    ...
    public CreditCardsResponse readCreditCards() {
        List<CreditCards> cards =
                loadCreditCardsUseCase.loadCreditCards(currentUserContext.getUserId());
        return new CreditCardsResponse(cards);
    }

    public CreditCardResponse saveCreditCard(RegisterCreditCardRequest request) {
        RegisterCreditCardCommand command =
                cardApiMapper.toRegisterCreditCardCommand(currentUserContext.getUserId(), request);
